...
Обновление 2026-09-25: [подстройка тока по АЦП с фиксацией ЦАП](docs/CURRENT_FEEDBACK.md).
Вкладка «Ток по платам» задаёт общий ток четырём каналам каждой платы;
кнопки «Установить все» и «Сбросить все» управляют несколькими платами.
После трёх измерений в заданном допуске программа прекращает коррекции.
Повторный подбор — кнопкой «Применить». Работает с существующей прошивкой.
